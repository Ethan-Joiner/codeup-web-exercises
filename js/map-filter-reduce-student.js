...
let newerArray = newArray.map(n => n * 3);
...
let trueArray = binary.filter(n => n == true);

// TO DO: filter through binary again and this time, return all the false
// values.

let falseArray = binary.filter(n => n == false);
...
let fruitArray = fruitsAndVeggies.filter(x => x.type === 'fruit');
console.log(fruitArray);

// TO DO: filter through the array of objects and return all vegetable type
// objects.

let veggieArray = fruitsAndVeggies.filter(x => x.type === 'vegetable');
